...
import { getSchoolSmtpConfig, saveSchoolSmtpConfig, testSchoolSmtpConfig } from '@/lib/actions/school-settings';
...
export default function SmtpSettingsForm({ schoolId }: { schoolId: string }) {
    ...
    const handleTest = async () => {
        ...
        setTesting(true);
        try {
            const res = await testSchoolSmtpConfig(schoolId, testEmail);
            if (res.success) {
            ...
            toast.error(error.message || 'An unexpected error occurred during test');
        } finally {
            setTesting(false);
        }
    };
        ...
        <div className="space-y-6 mt-6">
            ...
            {config && (
                <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
                    <h4 className="text-sm font-bold text-slate-800 dark:text-white mb-3">Test Configuration</h4>
                    <div className="flex flex-col sm:flex-row gap-3">
                        <input
                            ...
                            className="flex-1 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                        />
                        <button
                            onClick={handleTest}
                            disabled={testing}
                            className="px-6 py-2 bg-slate-800 text-white rounded-lg font-medium hover:bg-slate-900 transition-colors disabled:opacity-70 text-sm flex items-center justify-center gap-2"
                        >
