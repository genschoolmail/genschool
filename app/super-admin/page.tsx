    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Platform Overview</h1>
                    <p className="text-slate-500 mt-1 text-sm">Real-time metrics and system health across all tenants.</p>
                </div>
                <Link href="/super-admin/schools/new" className="px-4 sm:px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-semibold transition-all shadow-lg shadow-indigo-200 dark:shadow-none inline-flex items-center gap-2 text-sm sm:text-base w-full sm:w-auto justify-center">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left min-w-[500px]">
                                <thead className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700 text-slate-500 font-semibold uppercase text-xs tracking-wider">
                                    <tr>
                                        <th className="p-3 sm:p-4">School</th>
                                        <th className="p-3 sm:p-4">Subdomain</th>
                                        <th className="p-3 sm:p-4 hidden sm:table-cell">Users</th>
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                    {recentSchools.map((school) => (
                                        <tr key={school.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                            <td className="p-3 sm:p-4 font-medium text-slate-800 dark:text-white text-sm">{school.name}</td>
                                            <td className="p-3 sm:p-4 text-slate-500 font-mono text-xs">{school.subdomain}</td>
                                            <td className="p-3 sm:p-4 text-slate-600 dark:text-slate-400 hidden sm:table-cell">
